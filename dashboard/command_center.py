...
def _protected_mutation_allowed() -> bool:
...
def _fortress_dashboard_basic_auth():
    ...
    if path == "/api/operator/halt" and request.method == "POST" and _operator_token_valid():
        return None
...
def _operator_halt_post_allowed() -> bool:
    """Require either operator token or valid dashboard Basic auth for halt mutations."""
    return _protected_mutation_allowed()
...
def _valid_env_value(value: str) -> bool:
    return bool(value) and all(ch not in value for ch in ("\r", "\n", "\0"))


def _setup_mutation_allowed() -> bool:
    # First-run setup remains public. After setup, credential changes need operator auth.
...
def api_setup_save_keys():
    ...
    try:
        ...
        if not _valid_env_value(api_key) or not _valid_env_value(secret_key):
            return jsonify({"ok": False, "error": "Keys contain invalid characters."}), 400
