...
def _get_pair_label() -> str:
    assets = require_market_assets()
    return (assets.get("forex_hedger") or {}).get("pair_label") or ""

def _get_oanda_instrument() -> str:
    """
    Convert configured pair label into OANDA instrument format.
    """
    pair_label = _get_pair_label().strip().upper()
    if len(pair_label) == 6:
        return f"{pair_label[:3]}_{pair_label[3:]}"
    return pair_label

def _get_yf_proxy_ticker() -> str:
    """
    Convert configured pair label into yfinance proxy ticker.
    """
    pair_label = _get_pair_label().strip().upper()
    if not pair_label:
        return ""
    return f"{pair_label}=X"
...
def find_sniper_setup():
    ...
    # Fetch configured FX proxy chart data (pseudo-code)
...
def execute_sniper_trade(setup):
    ...
    instrument = _get_oanda_instrument()
    if not instrument:
        logging.warning("Forex sniper disabled: missing pair_label in config/market_assets.json")
        return
    order = MarketOrderRequest(instrument=instrument, units=lot_size)
...
def get_eurusd_data(period='1d', interval='15m'):
    """Fetch configured FX proxy historical data."""
    try:
        proxy = _get_yf_proxy_ticker()
        if not proxy:
            return pd.DataFrame()
        data = yf.download(proxy, period=period, interval=interval)
        return data
    except Exception as e:
        logging.error(f"Error fetching FX proxy data: {e}")
        return pd.DataFrame()
...
def calculate_position_size(account_balance, stop_pips=10):
    ...
    pip_value = 0.0001  # Standard pip value for FX pairs
