...
class DashboardSetupAuthTests(unittest.TestCase):
    ...
    def test_first_run_test_connection_remains_public_after_key_save(self):
        self._write_existing_keys()
        self.assertTrue(cc._setup_test_connection_allowed())

    def test_sealed_setup_requires_auth_for_test_connection(self):
        cc.DATA_DIR.mkdir(parents=True, exist_ok=True)
        cc.SETUP_COMPLETE_FILE.write_text("ok", encoding="utf-8")

        with cc.app.test_request_context("/api/setup/test_connection", method="POST"):
            self.assertFalse(cc._setup_test_connection_allowed())

        with cc.app.test_request_context(
            "/api/setup/test_connection",
            method="POST",
            headers={"Authorization": "Basic b3BlcmF0b3I6c3Ryb25nLXBhc3M="},
        ):
            self.assertTrue(cc._setup_test_connection_allowed())
